...
> 🚀 **Новичку — начни отсюда:** [INSTRUKCIYA-dlya-chajnikov.md](INSTRUKCIYA-dlya-chajnikov.md)
> — пошаговая инструкция «с нуля до работающего VPN» с мельчайшими деталями.
...
sudo make update        # обновить всё до свежих версий (Xray, Hysteria2)
make versions           # что установлено и что вышло нового
make help               # все команды
```

**Всегда свежее.** Установщик использует официальные скрипты XTLS и Hysteria —
каждая установка тянет последнюю стабильную версию. Обновить работающий сервер —
одна команда `sudo ./deploy.sh update` (см. [docs/11-obnovleniya.md](docs/11-obnovleniya.md)).
...
| [11-obnovleniya.md](docs/11-obnovleniya.md) | Обновления «на все случаи жизни» (одна команда) |
...
Репозиторий **полностью самодостаточный и безопасный для шеринга** — в нём нет
секретов (ключи, пароли и IP создаются на сервере при установке; `.env` и `out/`
исключены из git). Каждый коллега получает **свой независимый VPN со своими
ключами** — общих секретов не возникает.

Как поделиться:
1. Скинь коллеге ссылку на репозиторий (репозиторий должен быть **Public**, либо
   добавь его в Collaborators в настройках GitHub).
2. Коллега берёт **свой** VPS и проходит [INSTRUKCIYA-dlya-chajnikov.md](INSTRUKCIYA-dlya-chajnikov.md):
   `git clone … && bash bootstrap.sh` — получает собственный рабочий VPN за ~15 минут.
3. Обновления у каждого — своей командой `sudo ./deploy.sh update`.

> Один сервер можно использовать и **на несколько человек**: добавьте каждому
> отдельный `client` (свой UUID) — см. [docs/03-vless-reality.md](docs/03-vless-reality.md),
> раздел «Несколько пользователей». Так у каждого своя ссылка и видна статистика.
